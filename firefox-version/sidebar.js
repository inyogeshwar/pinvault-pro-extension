// Sidebar JavaScript for PinVault Pro (Firefox Version)
...
class PinVaultProSidebar {
    ...
    async updateStats() {
        try {
            ...
            const response = await browser.tabs.sendMessage(tab.id, { action: 'getImageCounts' });
    ...
    async selectAll() {
        try {
            ...
            try {
                ...
                await browser.tabs.sendMessage(tab.id, { action: 'selectAllImages' });
            } catch (error) {
    ...
    async deselectAll() {
        try {
            ...
            try {
                // Try using content script first
                await browser.tabs.sendMessage(tab.id, { action: 'deselectAllImages' });
            } catch (error) {
                // Fallback to direct script injection
                await browser.tabs.executeScript(tab.id, {
                    code: `
    ...
    async toggleAutoScroll(enabled) {
        try {
            ...
            if (enabled) {
                ...
                try {
                    await browser.tabs.sendMessage(tab.id, { action: 'startAutoScroll' });
                } catch (error) {
                ...
                try {
                    await browser.tabs.sendMessage(tab.id, { action: 'stopAutoScroll' });
                } catch (error) {
                    // Fallback to direct script injection
                    await browser.tabs.executeScript(tab.id, {
                        code: `
    ...
    cancelDownload() {
        browser.runtime.sendMessage({ action: 'cancelDownload' });
        this.hideProgress();
    }
