// Background script for PinVault Pro Extension (Firefox Version)
class PinVaultProBackground {
    ...
    setupDownloadHandlers() {
        ...
        // Note: downloads.onDeterminingFilename is not supported in Firefox
        // Using alternative download completion detection
    }
    ...
    handleTabUpdate(tabId, changeInfo, tab) {
        ...
        if (changeInfo.status === 'complete' && tab.url && this.isPinterestUrl(tab.url)) {
            // Firefox doesn't have chrome.scripting API, use tabs.executeScript
    ...
    async downloadSingleImage(imageData, settings) {
        try {
            ...
            // Firefox uses browser.downloads instead of chrome.downloads
    ...
    async sendMessageToAllExtensionPages(message) {
        try {
            ...
            for (const tab of tabs) {
                if (tab.url && (tab.url.includes(browser.runtime.id) || tab.url.includes('pinterest'))) {
